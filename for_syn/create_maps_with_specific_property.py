...
def main():
    ...
    for img_name in os.listdir(opt.data_path):
        if is_image(img_name):
           ...
           # B_patch = B
            ...
            # Rain_patch = O_patch-B_patch # Rain_patch is in rgb format
            # r, g, b = cv2.split(Rain_patch)
            # Rain_patch = cv2.merge([b, g, r])
            # cv2.imwrite((opt.save_patch + '/'+ 'rain'+ img_name),np.uint8(255 * Rain_patch))

            # execute latent space interpolation for patch image
            O = np.expand_dims(O_patch.transpose(2, 0, 1), 0) # rgb to brg
    ...
    #     # based on each dimension of z calculate 
